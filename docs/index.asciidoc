...
| <<plugins-{type}s-{plugin}-mark_read>> |<<boolean,boolean>>|No
...
Flag processed messages as deleted, the message will still be available on the server.
...
Expunge messages in the IMAP folder. This will permanently delete messages from the IMAP server.
...
[id="plugins-{type}s-{plugin}-mark_read"]
===== `mark_read`

  * Value type is <<boolean,boolean>>
  * Default value is `true`

Flag processed messages as read. Set this to `false` when uid_tracking is enabled
to avoid making any changes on the server.
